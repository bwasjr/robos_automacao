    ...
    if (redirecionaveis > 0):
        ...
        for x in range(total_incidentes):
            if(lista_grp_destino[x] != 'INDETERMINADO'):
                ...
                redireciona_incidente(driver, lista_ids[x], lista_grp_destino[x], expande, df['Tipo de Produto'][x], lista_tipificacao[x])#chama a função que redireciona o incidente passando o id
...
def redireciona_incidente(driver, id_incidente, grupo_destino, expande, tipo_produto, tipificacao):
    time.sleep(2)
    ...
    if (tipificacao == np.nan):#se tipificacao for nulo
        df_tipo_produto_abend = pd.read_excel('\\\\srv-arquivos07\\dirgerti\\SEDT_AUTORE\\TN_AUTORE\\SUSTENTAÇÃO\\Incidentes\\robo_sigs\\TIPO_PRODUTO_ABEND.xlsx', shee_tname='Planilha1') #data frame com a lista dos tipos de produto relacionados a abend
        if (df_tipo_produto_abend['TIPO_PRODUTO'].str.contains(tipo_produto).any()):
            tipificacao = 'INCIDENTE (ABEND/INTERRUPÇÃO)'#recebe tipificação de abend
        else:
           tipificacao = 'DISFUNÇÃO'
        text_area = IS.retorna_objetos(driver, 'id', 'X317')#recupera o seletor de tipificações
        time.sleep(1)
        IS.insere_texto(text_area, tipificacao)#insere o texto de tipificação
    time.sleep(1)
    driver.switch_to.default_content()#retorna ao content default
