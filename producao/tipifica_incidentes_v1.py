...
import robo_sigs_extrai_incidentes_v18 as EX
...
def inicia_tipificacao():
    ...
    df_incidentes = df_incidentes[df_incidentes['Brd Tp in'] != 'INCIDENTE (ABEND/INTERRUPÇÃO)']#seleciona somente os incidentes que não estão tipificados como abend
    df_incidentes = df_incidentes[df_incidentes['Brd Tp in'] != 'DISFUNÇÃO']#seleciona somente os incidentes que não estão tipificados como disfunção
    ...
    #manter o somente os incidentes que tenham o tipo de produto diferente de abend

    #REMOVER DEPOIS QUE CONSEGUIR O ACESSO A ESSES GRUPOS NO SIGS
    df_incidentes = df_incidentes[df_incidentes['Designação principal'] != 'DS - BS - SUSTENTACAO-BARE-SINISTRO-RE']#seleciona somente os incidentes que não estão tipificados como disfunção#remover depois que incluir meu usuário no grupo do sinistro RE
    df_incidentes = df_incidentes[df_incidentes['Designação principal'] != 'DS - BS - SUSTENTACAO-BARE-SINISTRO-AUTO']#seleciona somente os incidentes que não estão tipificados como disfunção#remover depois que incluir meu usuário no grupo do sinistro auto
    ...
    if (total_incidentes > 0):
        ...
        for x in range(total_incidentes):
            ...
            tipifica_incidente(driver, lista_ids[x], expande)#chama a função que tipifica o incidente passando o id
...
def tipifica_incidente(driver, id, expande):#driver, id do incidente, flag que determina se a aba 'atividade' será precisa ser expandida
    time.sleep(2)
    ...
    IS.insere_texto(IS.retorna_objetos(driver, 'id', 'X257'), 'CORRIGIR DIRECIONAMENTO')#insere o texto "Corrigir Direcionamento"
    time.sleep(1)
    text_area = IS.retorna_objetos(driver, 'id', 'X268View')
    time.sleep(1)
    IS.insere_texto(text_area, 'teste robô teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô  teste robô teste robô teste robô teste robô SIGS teste robô SIGS teste robô SIGS teste robô SIGS teste robô SIGS teste robô SIGS')#insere a justificativa do redirecionamento
    time.sleep(1)
    IS.retorna_objetos(driver, 'id', 'X317').clear()
    time.sleep(1)
    text_area = IS.retorna_objetos(driver, 'id', 'X317')
    time.sleep(1)
    IS.insere_texto(text_area, 'DISFUNÇÃO')#seleciona a tipificação
    time.sleep(1)
